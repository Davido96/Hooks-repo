...
interface ProfileModalNotSubscribedProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ProfileModalNotSubscribed({
  isOpen,
  onClose,
  onSubscribeClick,
  match,
}: ProfileModalNotSubscribedProps) {
  const { 
    followUser, 
    unfollowUser, 
    loading, 
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Action Buttons (Not Subscribed State) */}
        <div className="space-y-3 mb-6">
          ...
          <Button
            ...
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 h-auto text-base"
            disabled={loading}
          >
            <Key size={20} className="mr-2" /> Subscribe (
            {match.subscriptionFee} Keys)
          </Button>
          <Button
            onClick={handleSendMessage}
            variant="outline"
            className="w-full bg-white border-gray-300 text-gray-700 hover:bg-gray-50 flex items-center justify-center py-2 h-auto text-base"
            disabled={loading}
          >
